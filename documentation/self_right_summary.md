...
- **Actuation:** Rotate wing props-up (position PID → `DO_SET_ACTUATOR`), then **symmetric** thrust on both tractors → pure pitching moment. One law only: ramp to `SR_THR_MAX` over `SR_RAMP_T`, cut on over-center or `SR_TIMEOUT`.
